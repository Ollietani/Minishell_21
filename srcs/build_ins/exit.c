...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/24 12:10:35 by myael             #+#    #+#             */
/*   Updated: 2021/11/29 18:11:21 by myael            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exit_build_in(t_data *s)
{
	...
	if (!s->command[1])
		exit_minishell();
	check = check_if_str_num(s->command[1]);
	num = atoi_mod(s->command[1], &flag);
	if (!check || (check && !s->command[2] && flag == 1))
		print_error_exit(s->command[1]);
	else if (check && s->command[2])
	{
		ft_putendl_fd("bash: exit: too many arguments", 2);
		g_return_code = 1;
		return ;
	}
	else if (check && !s->command[2] && flag == 0)
